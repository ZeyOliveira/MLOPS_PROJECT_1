...
class DataIngestion:
    def __init__(self, config):
        ...
        self.test_train_ratio= self.config["train_ratio"]

        os.makedirs(RAW_DIR, exist_ok=True)

        logger.info(
            f"Data ingestion started with {self.bucket_name} and file is {self.bucket_file_name}"
        )
    ...
    def download_csv_from_gcp(self):
        try:
            ...
            logger.info(f"CSV file is succesfully dowloaded to {RAW_FILE_PATH}")

        except Exception as e:
            logger.error(f"Error while dowloading the csv file")
    ...
    def split_data(self):
        try:
            ...
            train_data, test_data = train_test_split(
                data, test_size=1-self.test_train_ratio, random_state=RANDOM_STATE
            )
